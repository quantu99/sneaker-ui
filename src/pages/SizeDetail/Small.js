...
function SmallItems({ product, handleAddProducts, handleAddWishProducts }) {
...
function Small({ cartItems, handleAddProducts, wishItems, handleAddWishProducts, productItems }) {
    ...
    useEffect(() => {
        const products = productItems.filter((products) => {
            return products.size > 35 && products.size <= 38;
        });
        setSize(products);
    }, []);
    return (
                        ...
                        <ul>
                            <li className={cx('brand')}>
                                <Link className={cx('checkbox')} to={'/sneakers/1000000'}>
                                    <FontAwesomeIcon className={cx('square', 'less-one')} icon={faSquare} />
                                    <FontAwesomeIcon
                                        className={cx('square-check', 'less-one-check')}
                                        icon={faSquareCheck}
                                    />
                                </Link>
                                &#60; đ 1.000.000
                            ...
                            <ul>
                                <li className={cx('brand')}>
                                    <Link className={cx('checkbox')} to={`/sneakers/small`}>
                                        <FontAwesomeIcon className={cx('square', 'small')} icon={faSquare} />
                                        <FontAwesomeIcon
                                            className={cx('square-check', 'small-check')}
                                            icon={faSquareCheck}
                                        />
                                    </Link>
                                    36- 38(US: 3.5- 5.5)
